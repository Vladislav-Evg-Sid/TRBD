...
namespace DataBase
{
    public partial class insEditRent : Form
    {
        ...
        public insEditRent(string name, string ConnectionString)
        {
            ...
            a = new SQLiteDataAdapter("SELECT ID_Employee || '. '|| Employee_Surname || ' ' || SUBSTR(Employee_Name, 1, 1) || '.' AS [Emp] FROM Employee", sc);
            a.Fill(tmp.Tables["Employee"]);
            comboBox1.DisplayMember = tmp.Tables["Employee"].Columns["Emp"].ToString();
            comboBox1.ValueMember = tmp.Tables["Employee"].Columns["Emp"].ToString();
            comboBox1.DataSource = tmp.Tables["Employee"];
            tmp = new DataSet();
            tmp.Tables.Add(new DataTable("Client"));
            a = new SQLiteDataAdapter("SELECT ID_Client || '. '|| Client_Surname || ' ' || SUBSTR(Client_Name, 1, 1) || '.' AS [Cli] FROM Client", sc);
            a.Fill(tmp.Tables["Client"]);
            comboBox2.DisplayMember = tmp.Tables["Client"].Columns["Cli"].ToString();
            comboBox2.ValueMember = tmp.Tables["Client"].Columns["Cli"].ToString();
            comboBox2.DataSource = tmp.Tables["Client"];
            tmp = new DataSet();
            tmp.Tables.Add(new DataTable("Auto"));
            a = new SQLiteDataAdapter("SELECT Auto.ID_Auto || '. '|| Brand.Brand || ' ' || Model.Model AS [Aut] FROM Auto JOIN Brand ON Auto.ID_Brand = Brand.ID_Brand JOIN Model ON Auto.ID_Model = Model.ID_Model", sc);
            a.Fill(tmp.Tables["Auto"]);
            comboBox3.DisplayMember = tmp.Tables["Auto"].Columns["Aut"].ToString();
            comboBox3.ValueMember = tmp.Tables["Auto"].Columns["Aut"].ToString();
